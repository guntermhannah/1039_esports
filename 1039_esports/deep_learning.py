...
def get_data():
    ...
    X = merged.drop(columns = ["match_id", "player", "opponent", "winner", "player_win", "player_tower_damage", "player_roshans_killed", "player_obs_placed", "opponent_tower_damage","opponent_roshans_killed","opponent_obs_placed"])
    y = merged["player_win"]
...
def init_model(X, y):
    ...
    # fit model
    history = model.fit(X,y, batch_size = 32, validation_split = 0.3, epochs = 300, verbose = True)
